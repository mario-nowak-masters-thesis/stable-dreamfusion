...
@torch.cuda.amp.autocast(enabled=False)
def get_better_variable_resolution_rays(poses, intrinsics, H: int, W: int, render_height: int, render_width: int):
    ''' get rays
    Args:
        poses: [B, 4, 4], cam2world
        intrinsics: [4]
    ...
    i, j = custom_meshgrid(torch.linspace(0, W-1, render_width, device=device), torch.linspace(0, H-1, render_height, device=device))
    i = i.t().reshape([1, render_height*render_width]).expand([B, render_height*render_width]) + 0.5
    j = j.t().reshape([1, render_height*render_width]).expand([B, render_height*render_width]) + 0.5
...
class Trainer(object):
    ...
    def train_step(self, data, save_guidance_path:Path=None): # ! So this is potentially executed in autocast
        """
            Args:
    ...
    def train_one_epoch(self, loader, max_epochs):
        ...
        for data in loader:
            ...
            with torch.cuda.amp.autocast(enabled=self.fp16): # ! the training step uses autocast!
